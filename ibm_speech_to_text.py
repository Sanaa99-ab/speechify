...
def upload_file():
    try:
        ...
        if file:
            ...
            content_type = "audio/wav"
            selected_model = models.get(request.form.get('model'))
            if selected_model:
                ...
                return "Modèle de langue invalide."
    except Exception as e:
...
def convert_audio_to_text(audio_file):
    try:
        with open(audio_file, 'rb') as audio_file:
            response = speech_to_text.recognize(audio=audio_file, content_type='audio/wav').get_result()
            if 'results' in response and response['results']:
                recognized_text = response['results'][0]['alternatives'][0]['transcript']
                return recognized_text
            else:
                return 'Conversion failed: No results found'
    except Exception as e:
